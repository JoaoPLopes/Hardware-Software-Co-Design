...
/*syncronização flag*/
volatile int * p0Flag = (int *) 0xFFFF0000;
volatile int * p1Flag = (int *) 0xFFFF0004;

// Matrix multiplication: C = A * B
void gemm(float *A, float *B, float *C, float *bias, int rowsA, int colsA, int colsB)
{
  *sync_f = PROC0_STARTED;
  while (*sync_f != PROC1_STARTED); // Wait for P1
  int i, j, k;
  ...
  Xil_DCacheFlushRange((INTPTR)((float *)C), (unsigned)(4*5));
...
void prepare_matrixA()
{
  ...
  //Xil_DCacheFlushRange((INTPTR)((float *)matA), (unsigned)(4* 24*24*25*(1/2)));
  //while (*sync_f != PROC1_COMPLETED); // Wait for P1
}
...
int dmammBT(float *memA, float *memB, float *memTC, int rowsA, int colsA, int rowsB, float* bias)
{
  int Status;
  int row;
  ...
  float *biasBufferPtr;
  XTime tStart, tEnd;
  Xil_DCacheFlushRange((INTPTR)(memB), (unsigned)(4*25*22));
  Xil_DCacheFlushRange((INTPTR)(memA), (unsigned)(4*25*24*24));
  //printf("\n=====Matrix A\n");
  //print_fp_mat((float *)memA, rowsA, colsA);
  //printf("\n=====Matrix B\n");
  //print_fp_mat((float *)memB, rowsB, colsA);
 XTime_GetTime(&tStart);
 for (row=0; row < rowsB; row++) {

    // send row of B (column of TB)
    TxBufferPtr = (float *)memB + (row * colsA);
    Status = XAxiDma_SimpleTransfer(&AxiDma,(UINTPTR) TxBufferPtr,
				    (colsA*4), XAXIDMA_DMA_TO_DEVICE);
    if (Status != XST_SUCCESS) { return XST_FAILURE; }

    while (XAxiDma_Busy(&AxiDma, XAXIDMA_DMA_TO_DEVICE)) {/* Wait for Tx*/ }


    biasBufferPtr = (float *)&bias[row];
    	Status = XAxiDma_SimpleTransfer(&AxiDma,(UINTPTR) biasBufferPtr,
    					    (4), XAXIDMA_DMA_TO_DEVICE);
    	    if (Status != XST_SUCCESS) { return XST_FAILURE; }

    // receive row of TC (column of C)
    RxBufferPtr = (float *)memTC + (row * rowsA);
    Status = XAxiDma_SimpleTransfer(&AxiDma,(UINTPTR) (RxBufferPtr),
				    (rowsA*4), XAXIDMA_DEVICE_TO_DMA);
    if (Status != XST_SUCCESS) { return XST_FAILURE; }

    // send full matrix A
    TxBufferPtr = (float *)memA;
    Status = XAxiDma_SimpleTransfer(&AxiDma,(UINTPTR) TxBufferPtr,
				      (rowsA*colsA*4), XAXIDMA_DMA_TO_DEVICE);
    if (Status != XST_SUCCESS) { return XST_FAILURE; }
  ...
  Xil_DCacheInvalidateRange((INTPTR)(memTC), (unsigned)(4 * rowsB * rowsA));

  //printf("\n=====Matrix TC\n");
  //print_fp_mat((float *)memTC, rowsB, rowsA);
...
void forward_maxpool_layer()
{
  *sync_f = PROC0_STARTED;
  ...
  Xil_DCacheFlushRange((INTPTR)(pout), (unsigned)(4* 11*12*12));
  while (*sync_f != PROC1_COMPLETED); // Wait for P1
...
void forward_convolutional_layer()
{
    ...
    if(dmammBT((float *)matA, (float *)matB, (float *)matCbias, 24*24, 25, 22, (float *)fp_weights)!=XST_SUCCESS)
    	printf("DMA_ERROR");
    // Add bias and transpose.
    //add_bias((float *)matC, 22, 24*24, (float *)fp_weights, (float *)matCbias, 0);
...
int predict_mnist()
{
  ...
  printf("Total Time took %.0f us \n", ptime[0]+ptime[1]+ptime[2]+ptime[3]);
#endif
  return best;
}
